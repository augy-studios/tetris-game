...
//   best  (default) -> { board, entries: [{ rank, name, score, lines }] }
...
const BOARDS = {
  best: {
    query: `uwutetris_leaderboard_best?select=name,score,lines&order=score.desc,created_at.asc&limit=${LIMIT}`,
    row: (r) => ({ name: r.name, score: r.score, lines: r.lines }),
  },
  total: {
    query: `uwutetris_leaderboard_total?select=name,total,games&order=total.desc,games.asc,last_at.asc&limit=${LIMIT}`,
